package com.sparta.postingboard.jwt;
...
public class JwtUtil {
    ...
    private Key key;  // app.property에서 받아온 시크릿 키를 담을 것임
    private final SignatureAlgorithm signatureAlgorithm = SignatureAlgorithm.HS256;

    // 로그 설정
    public static final Logger logger = LoggerFactory.getLogger("JWT 관련 로그");

    @PostConstruct   // 딱 한번만 받아와서 사용하는 값을 받아올때 마다 요청을 새로 하는 실수를 방지하고자 붙이는 애노테이션
    public void init() {
    ...
    public String substringToken(String tokenValue) {
        if (StringUtils.hasText(tokenValue) && tokenValue.startsWith(BEARER_PREFIX)) {  // hasText 공백인지 확인 , startsWith- bearer로 시작하는지 확인
            return tokenValue.substring(7);  //BEARER_ 까지 7자 이므로 beginIndex는 7
        }
    ...
    public boolean validateToken(String token) {
        try {
            Jwts.parserBuilder().setSigningKey(key).build().parseClaimsJws(token); // 토큰 위변조 식별
            return true;
    ...
    // jwt에서 사용자 정보 가져오기
    public Claims getUserInfoFromToken(String token) { // getbody부분에 들어있는 Claims 를 반환
